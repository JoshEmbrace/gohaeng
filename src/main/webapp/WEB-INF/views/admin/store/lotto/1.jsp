		...
		<div>
			<div class="content_wrap content_seller_info content_seller_645">
					...
					<div class="box_option box_option1">
						...
						<div class="forms">
							<form id="frmSrch1" name="frmSrch1" method="post">
								<input type="hidden" name="searchType" value="1"> <input type="hidden" id="nowPage1"
					...
					<div class="box_option box_option2">
						...
						<div class="forms">
							<form id="frmSrch2" name="frmSrch2" method="post" onsubmit="false">
								<input type="hidden" name="searchType" value="2"> <input type="hidden" id="nowPage2"
									name="nowPage"> <select id="kind" name="kind" title="상호/지역(읍/면/동) 선택">
									<option value="0">상호</option>
									<option value="1">지역(읍/면/동)</option>
								</select> <input type="text" name="srchVal" id="srchVal" maxlength="20" value=""
									onkeydown="" onchange="clearSlt()" title="상호/지역(읍/면/동) 입력"> <a
				...
				<form id="frmSrch3" name="frmSrch3" method="post">
					<input type="hidden" name="searchType" value="3"> <input type="hidden" id="nowPage3" name="nowPage">
					<input type="hidden" id="sltSIDO2" name="sltSIDO2" value=""> <input type="hidden" id="sltGUGUN2"
						name="sltGUGUN2" value="">
				</form>
						...
						<div class="action">
							<div class="search">
								<form id="frmSrch4" name="frmSrch4" method="post">
									<input type="hidden" name="searchType" value="4"> <input type="hidden" id="nowPage4"
										name="nowPage"> <select id="rtlrSttus" name="rtlrSttus" title="조회옵션 선택">
				...
				<table class="tbl_data tbl_data_col" id="resultTable">
					...
					<tbody>
						<c:forEach var="list" items="${list}">
							<tr>
								<td class="ta_left">${list.STORE_NAME}</td>
								<td>${list.STORE_TEL}</td>
								<td class="ta_left">${list.STORE_ADDR}</td>
								<td><a class="btn_search"
										onclick="window.open('1detail?STORE_INDEX=${list.STORE_INDEX}','a', 'resizable=no width=700 height=800');return false"
										href="1detail?STORE_INDEX=${list.STORE_INDEX}" title="새창">위치보기</a></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
				<div class="btn_common form write">
					<a onclick="window.open('1insertform','a', 'resizable=no width=700 height=800');return false"
						title="새창">글쓰기</a>
	...
	private String STORE_ADDR; -->
		<script>
			$(function () {
				...
				$.getJSON("${pageContext.request.contextPath}/map/ajax", function (data) {
					var a = "<tr><td class='ta_left'>" + data.STORE_NAME + "</td><td>" + data.STORE_TEL + "</td><td class='ta_left'>" + data.STORE_ADDR + "</td><td><a class='btn_search' href='#' title='새창'>위치보기</a></td></tr>"
